...
@section('content')
    <div class="container">
        ...
        <div class="btn-group tab">
            <a class="btn btn-default active" href="/org_invite_codes">机构</a>
            <a class="btn btn-default" href="/teach_invite_codes">教师</a>
        </div>

        <form class="clearfix form-group form-inline" action="/org_invite_codes" method="get">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group pull-right">
                                    ...
                                    @endif
                                    <button type="button" class="btn btn-xs btn-info edit_institution" data-toggle="modal" data-target="#edit_institution" data-id="{{ $v->id }}">编辑</button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
...
@section('js')
    ...
    $(document).ready(function() {
        ...
        $(".release_invite_code").click(function() {
            $.post('/institution/' + $(this).attr('data-id') + '/invite_code', {invite_code: 1, _method: 'put', _token: $("input[name=_token]").val()}, function(json, textStatus) {
                location.reload();
            });
        });
        ...
        $(".edit_institution").click(function() {
            $.getJSON('/institution/' + $(this).attr('data-id'), function(json, textStatus) {
                    $("#edit_institution input[name=institution_name]").val(json.data.name);
                    $("#edit_institution input[name=cell_phone]").val(json.data.cell_phone);
                    $("#edit_institution input[name=address]").val(json.data.address);
                    $("#edit_institution input[name=email]").val(json.data.email);
                    $("#edit_institution form").attr('action', '/institution/' + json.data.id);
            });
        });

        // 保存机构信息
        $("#save_ins_info").click(function() {
            $("#edit_institution form").submit();
        });

        // 保持select中上次查询的条件
        $("select[name=province]").val("{{ Input::get('province', '') }}");
        $("select[name=institution]").val("{{ Input::get('institution', '') }}");
